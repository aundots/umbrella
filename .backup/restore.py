...
def rclone(args):
    ...
    result = subprocess.run([command, '--config', str(STATE/'rclone.conf'), '--contimeout', '20s', '--timeout', '60s', *args], capture_output=True, env=env, timeout=1800 if args and args[0]=='copyto' else 240)
