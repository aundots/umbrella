...
The original offline recovery master key stays on Windows/offline storage. Codespaces user secrets hold separate per-project derived keys, restricted to the corresponding repository. Drive OAuth credentials are user secrets restricted to these six repositories. This personal OAuth credential uses drive.file and can access files created or explicitly opened with the new backup app; do not grant these secrets to untrusted repositories or run untrusted code with them. Archives are encrypted and authenticated. Original Windows snapshots retain their original namespace/key; hybrid snapshots use `Codex-Private-Backups/hybrid` and `hybrid-history`. Keep both archive namespaces and the offline key.
...
Windows hybrid backup uses the separate encrypted personal-drive.conf. The original rclone.conf remains available for legacy recovery; keep both configurations. The personal OAuth app is described at https://aundots.github.io/project-secret-backup-docs/.
